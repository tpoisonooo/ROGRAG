...
class TestChunkSQL(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.chunk = Chunk(_hash='test_hash', content_or_path='test_content', metadata={"key":"value"}, modal='text')

    def test_init(self):
        # 测试初始化是否创建了目录和数据库
        self.assertTrue(os.path.exists(self.file_dir))
        self.assertTrue(os.path.exists(os.path.join(self.file_dir, 'chunks.sql')))
    ...
    def test_add_get_chunk(self):
        ...
        self.assertEqual(retrieved_chunk.content_or_path, self.chunk.content_or_path)

...
def build_low_high_level_db(work_dir:str, resource: RetrieveResource):
    ...
    entityDB.save(folder_path=os.path.join(work_dir, 'db_kag_entity'), embedder=resource.embedder)
    
    relationDB = Faiss()
    relationDB.upsert(Chunk(content_or_path="教书"))
    relationDB.upsert(Chunk(content_or_path="成长的地方"))
    relationDB.save(folder_path=os.path.join(work_dir, 'db_kag_relation'), embedder=resource.embedder)
...
class TestKAG(unittest.IsolatedAsyncioTestCase):
    ...
    def test_combine_contexts(self):
        high_level_context = type('RetrieveReply', (), {"text_units": [["entity1"]], "nodes": [["entity1"]], "relations": [["entity1"]]})
        low_level_context = type('RetrieveReply', (), {"text_units": [["entity2"]], "nodes": [["entity2"]], "relations": [["entity2"]]})
        result = self.kag.combine_contexts(high_level_context, low_level_context)
        assert result.text_units == [["entity1"], ["entity2"]]
        
    async def test_query_local_query(self):
        query = Query(text='花园, 我家后面, 相传')
        ret = await self.kag._build_local_query_context(query, self.kag.graph, self.kag.entityDB, self.kag.chunkDB)
        
    async def test_query_global_query(self):
        query = Query(text='花园, 我家后面, 相传')
        ret = await self.kag._build_global_query_context(query, self.kag.graph, self.kag.entityDB, self.kag.relationDB, self.kag.chunkDB)

...
class MathExecutor(OpExecutor):
    ...
    async def run(self, logic_node: LogicNode, op_sess: OpSession):
        ...
        # build params
        param_list_str = '参数列表{}\n'.format(json.dumps(logic_node.set, ensure_ascii=False, default=default_json))
        _, vars_, _ = self.split(logic_node.set)
        param_dict_str = ''
        for var_ in vars_:
            param_dict_str += '参数内容{}\n'.format(json.dumps({var_:op_sess.param.get(var_)}, ensure_ascii=False, default=default_json))
        
        param_text = "{param_list_str}{param_dict_str}".format(param_list_str=param_list_str, param_dict_str=param_dict_str)
        step_text = json.dumps(op_sess.evidence_strs, ensure_ascii=False)
        prompt = PROMPTS['math']['zh_cn'].format(task=task, param_text=param_text, step_text=step_text, sub_query=logic_node.sub_query, root_query=logic_node.root_query)
        
        ret = await self.resource.llm.chat(prompt)
        op_sess.param[logic_node.alias_name] = str(ret)
        op_sess.upsert_evidence(sub_query=logic_node.sub_query, alias=logic_node.alias_name, items=[], sub_answer=str(ret))
        return None

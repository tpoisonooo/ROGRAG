...
class KnowledgeRetriever(Retriever):
    ...
    def __init__(self, resource: RetrieveResource, work_dir: str) -> None:
        ...
        self.relationDB = Faiss.load_local(
            os.path.join(work_dir, 'db_kag_relation_mix'))
    ...
    def combine_contexts(self, high_level_context: RetrieveReply,
                         low_level_context: RetrieveReply,
                         query: Query) -> RetrieveReply:
        # Function to extract entities, relationships, and sources from context strings

        def process_combine_contexts(hl: List[Any], ll: List[Any],
                                     hash_func: callable) -> List[List[str]]:
            keys = set()
            rets = []
            for item in hl:
                key = hash_func(item)
        ...
        nodes = process_combine_contexts(hl=high_level_context.nodes,
                                         ll=low_level_context.nodes,
                                         hash_func=lambda x: x[0])
        relations = process_combine_contexts(hl=high_level_context.relations,
                                             ll=low_level_context.relations,
                                             hash_func=lambda x: x[0] + x[1])
        sources = process_combine_contexts(hl=high_level_context.sources,
                                           ll=low_level_context.sources,
                                           hash_func=lambda x: x._hash)

        nodes = truncate_list_by_token_size(
            nodes, key=str, max_token_size=query.max_token_for_local_context)
        relations = truncate_list_by_token_size(
            relations,
            key=str,
            max_token_size=query.max_token_for_global_context)
        sources = truncate_list_by_token_size(
            sources, key=str, max_token_size=query.max_token_for_text_unit)
    ...
    async def _find_most_related_text_unit_from_entities(
        ...
        for index, (this_text_units,
                    this_edges) in enumerate(zip(text_units, edges)):
            for c_id in this_text_units:
                if c_id in all_text_units_lookup:
                    continue
                relation_counts = 0
                for e in this_edges:
                    if (e._tid in all_one_hop_text_units_lookup
        ...
        all_text_units = [{
            "id": k,
            **v
        } for k, v in all_text_units_lookup.items() if v is not None]
        all_text_units = sorted(all_text_units,
                                key=lambda x:
                                (x["order"], -x["relation_counts"]))
    ...
    async def _build_local_query_context(self, query: Query,
        ...
        node_datas = [
            knowledge_graph_inst.get_node(c.metadata["entity_name"])
            for c in chunks
        ]
        ...
        node_degrees = await asyncio.gather(*[
            knowledge_graph_inst.node_degree(c.metadata["entity_name"])
            for c in chunks
        ])

        node_datas = [{
            **n.props, "entity_name": k.metadata["entity_name"],
            "rank": d
        ...
        r = RetrieveReply(nodes=entites_section_list,
                          relations=relations_section_list,
    ...
    async def _build_global_query_context(
        ...
        edge_datas = [
            knowledge_graph_inst.get_edge(c.metadata["src_id"],
                                          c.metadata["tgt_id"]) for c in chunks
        ]
        ...
        if not all([n is not None for n in edge_datas]):
            logger.warning(
                "Some edges are missing, maybe the storage is damaged")

        edge_datas = [list(edge_data)[0] for edge_data in edge_datas]
        edge_degree = await asyncio.gather(*[
            knowledge_graph_inst.edge_degree(
                c.metadata["src_id"], c.metadata["tgt_id"]) for c in chunks
        ])
        edge_datas = [{
            "src_id": v.sid,
            "tgt_id": v.tid,
            "rank": d,
            **v.props, "keywords": v.name
        } for v, d in zip(edge_datas, edge_degree) if v is not None]
        edge_datas = sorted(edge_datas,
                            key=lambda x: (x["rank"], x["weight"]),
                            reverse=True)
    ...
    async def explore(self, query: Query) -> RetrieveReply:
        ...
        if hl_keywords:
            results = self.relationDB.similarity_search(
                embedder=self.embedder,
                query=query,
                threshold=self.DENSE_THRESHOLD)
            if results:

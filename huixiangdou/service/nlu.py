...
def clean_str(input: Any) -> str:
    ...
    cleaned_content = re.sub(r"[\x00-\x1f\x7f-\x9f]", "",
...
def truncate_list_by_token_size(list_data: list, key: callable,
                                max_token_size: int):
...
def split_string_by_multi_markers(content: str,
...
async def _merge_nodes_then_upsert(
    ...
    if already_node is not None:
        ...
        already_source_ids.extend(
            split_string_by_multi_markers(already_node.props.get("source_id"),
                                          [GRAPH_FIELD_SEP]))
        already_description.append(already_node.props.get("description"))

    entity_type = sorted(
        Counter([dp["entity_type"]
                 for dp in nodes_data] + already_entitiy_types).items(),
        key=lambda x: x[1],
        reverse=True,
    )[0][0]
...
async def _merge_edges_then_upsert(
    ...
    if knowledge_graph_inst.has_edge(src_id, tgt_id):
        ...
        already_source_ids.extend(
            split_string_by_multi_markers(already_edge["source_id"],
                                          [GRAPH_FIELD_SEP]))
    ...
    description = GRAPH_FIELD_SEP.join(
        sorted(
            set([dp["description"]
                 for dp in edges_data] + already_description)))
    ...
    for need_insert_id in [src_id, tgt_id]:
        ...
        if not (knowledge_graph_inst.has_node(need_insert_id)):
            knowledge_graph_inst.upsert_node(name=need_insert_id,
                                             data={
                                                 "source_id": source_id,
                                                 "description": description,
                                                 "entity_type": '"UNKNOWN"',
                                             })
    description = await _handle_entity_relation_summary((src_id, tgt_id),
                                                        description, llm)
    knowledge_graph_inst.upsert_edge(src_id,
                                     tgt_id,
                                     name=keywords,
                                     data=dict(
                                         weight=weight,
...
async def parse_chunk_to_knowledge(chunks: List[Chunk], llm: LLM,
                                   entityDB: Faiss, relationDB: Faiss,
                                   entityDB_mix: Faiss, relationDB_mix: Faiss,
    ...
    async def _process_single_content(chunk: Chunk):
        ...
        for record in records:
            ...
            if if_relation is not None:
                maybe_edges[(if_relation["src_id"],
                             if_relation["tgt_id"])].append(if_relation)
        already_processed += 1
        already_entities += len(maybe_nodes)
        already_relations += len(maybe_edges)
        now_ticks = PROMPTS["process_tickers"][already_processed %
                                               len(PROMPTS["process_tickers"])]
        print(
    ...
    if entityDB is not None:
        for dp in all_entities_data:
            entityDB_mix.upsert(
                Chunk(content_or_path=dp["entity_name"] + dp["description"],
                      metadata={
                          "entity_name": dp["entity_name"],
                          "entity_type": dp["entity_type"]
                      }))
            entityDB.upsert(
                Chunk(content_or_path=dp["entity_name"],
                      metadata={
                          "entity_name": dp["entity_name"],
                          "entity_type": dp["entity_type"],
    ...
    if relationDB is not None:
        for dp in all_relationships_data:
            relationDB_mix.upsert(
                Chunk(content_or_path=dp["keywords"] + dp["src_id"] +
                      dp["tgt_id"] + dp["description"],
                      metadata={
                          "src_id": dp["src_id"],
                          "tgt_id": dp["tgt_id"]
                      }))
            relationDB.upsert(
                Chunk(content_or_path=dp["keywords"],
                      metadata={
                          "src_id": dp["src_id"],
                          "tgt_id": dp["tgt_id"],
                          "description": dp["description"]

...
class Entity2ChunkSQL:
    ...
    def insert_relation(self, entity: str, chunk_ids: Union[List[str], str]):
        ...
        self.cursor.execute('INSERT or IGNORE INTO entities (entity, chunk_ids) VALUES (?, ?)', (entity, chunk_ids_str))
        self.conn.commit()
    ...
    def get_chunk_ids(self, entities: List[str]) -> Set:
        ...
        for entity in entities:
            if self.ignore_case:
                entity = entity.lower()
            self.cursor.execute('SELECT chunk_ids FROM entities WHERE entity = ?', (entity,))

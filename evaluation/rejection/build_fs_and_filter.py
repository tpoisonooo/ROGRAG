...
#     # 读 input.jsonl 计算 F1
#     cache = SharedRetrieverPool(config_path=config_path)
...
#     retriever = SharedRetrieverPool(config_path=config_path).get(
...
def calculate(chunk_size: int):
    ...
    # 按不同 chunk_size 和 chunk_size，构建特征库
    # 读 input.jsonl 计算 F1
    pool = SharedRetrieverPool(config_path=config_path)
    fs_init = FeatureStore(embedder=pool.embedder,
                           config_path=config_path,
                           chunk_size=chunk_size)
